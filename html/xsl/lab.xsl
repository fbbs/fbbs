...
<xsl:template name="_showpost">
	...
	<xsl:variable name='line'><xsl:call-template name='replace-space'><xsl:with-param name='str' select='$before'/></xsl:call-template></xsl:variable>
...
<xsl:template name='replace-space'>
	<xsl:param name='str'/>
	<xsl:choose>
		<xsl:when test='contains($str, " ")'><xsl:value-of select='substring-before($str, " ")'/><xsl:text>&#160;</xsl:text><xsl:call-template name='replace-space'><xsl:with-param name='str' select='substring-after($str, " ")'/></xsl:call-template></xsl:when>
		<xsl:otherwise>
