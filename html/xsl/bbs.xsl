...
<xsl:template match='bbsqry'>
	...
	<xsl:choose><xsl:when test='@login'><div class='post'>
		...
		<p>文章数:【<span class='a132'><xsl:value-of select='@post'/></span>】 贡献 【<span class='a132'><xsl:value-of select='@contrib'/></span>】(<xsl:value-of select='@rank'/>%)<xsl:if test='@money'> 财富 【<span class='a132'><xsl:value-of select='@money'/></span>】</xsl:if> 生命力:【<span class='a132'><xsl:value-of select='@hp'/></span>】</p>
