	...
	<xsl:template match="bbscon">
		<html>
			...
			<body>
				...
				<a><xsl:attribute name='href'>ccc?bid=<xsl:value-of select='bid' />&amp;f=<xsl:value-of select='f' /></xsl:attribute>[ 转载 ]</a>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template name='show-linkbar'>
		<xsl:if test='link = "con"'>
			<div>
				...
				<xsl:if test='gid'><a><xsl:attribute name='href'>con?bid=<xsl:value-of select='bid' />&amp;f=<xsl:value-of select='gid' /></xsl:attribute>[ 同主题第一篇 ]</a></xsl:if>
			</div>
		</xsl:if>
	</xsl:template>
</xsl:stylesheet>
