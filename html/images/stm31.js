...
//if(nIE4||nNN4)onerror=function(m,u,l){return !confirm("Java Script Error\n"+"\nDescription:"+m+"\nSource:"+u+"\nLine:"+l+"\n\nSee more details?");}
if(nIE4||nNN4)onerror=function(m,u,l){return false;}
if(nDM)onload=st_onload;
if(nIEM||nOP5)onunload=function(){if(st_rl){clearInterval(st_rl);st_rl=null;}for(var i=0;i<st_ms.length;++i)st_ms[i].cfrm=0;return true;}
...
function stshst(i,n)
{
	...
	else
	{
		...
		{
			t=nNN6?stgobj(i.ids+"ICON"):o.all[i.ids+"ICON"];
			if(t)	t.src=i.iicn[n];
		}
		if(i.iarr[0]!=i.iarr[1])
		{
			t=nNN6?stgobj(i.ids+"ARROW"):o.all[i.ids+"ARROW"];
			if(t)	t.src=i.iarr[n];
		}
		if(i.ityp==2&&i.iimg[0]!=i.iimg[1])
		{
			t=nNN6?stgobj(i.ids+"IMG"):o.all[i.ids+"IMG"];
			if(t)	t.src=i.iimg[n];
		}
		if (!i.txstyle)	i.txstyle=nNN6?stgobj(i.ids+"TX"):o.all[i.ids+"TX"].style;
		t=i.txstyle;
		if(i.itxf[0]!=i.itxf[1])
			t.font=i.itxf[n];
		if(i.itxd[0]!=i.itxd[1])
			t.textDecoration=i.itxd[n];
		if(nOP)	o.all[i.ids+'MTD'].style.color=i.itxc[n];
...
function stpre(m)
{
	...
	if(m.mtyp!=2)
		stshow(m.ps[0]);
	if(nIE)
		onscroll=new Function("if(st_scr)clearTimeout(st_scr);st_scr=setTimeout('stscr();',nIEM ?500:0);");
	else if(!st_rl)
	{
		st_cw=stgcw();
		st_ch=stgch();
...
function stgcl()
{
	return parseInt(nNN||nOP ? pageXOffset : document.body.scrollLeft);
}
function stgct()
{
	return parseInt(nNN||nOP ? pageYOffset : document.body.scrollTop);
}
function stgcw()
{
	return parseInt(nNN||nOP ? innerWidth : (nIEW&&document.compatMode=="CSS1Compat" ? document.documentElement.clientWidth : document.body.clientWidth));
}
function stgch()
{
	return parseInt(nNN||nOP ? innerHeight : (nIEW&&document.compatMode=="CSS1Compat" ? document.documentElement.clientHeight : document.body.clientHeight));
}
function stgobj(id)
{
	if(nIE&&nVER<5)
...
function stglay(ip)
{
	...
	{
		if(typeof(ip.iti)=='undefined')
			ip.layer=stgobj(ip.ids);
		else if(nNN4)
			ip.layer=stglay(stgpar(ip)).document.layers[0].document.layers[ip.ids];
		else if(nNN6)
			ip.layer=stgobj(ip.ids);
		else
			ip.layer=stglay(stgpar(ip)).all.tags("table")[ip.ids];
	}
	return ip.layer;
}
function stgstlay(i)
...
function stgtsub(i)
{
	...
	if(m.mcfb)
	{
		...
		if(i.ppi||w==window)
			return stgsub(i);
		if(typeof(w.stmenu)!="undefined"&&typeof(w.stgsub)!="undefined")
		{
			m=w.stmenu(m.mnam);
