...
static int read_submit(void)
{
	...
	mmap_close(&m);
	xml_header("bbsmybrd");
	printf("<root><bbsmybrd p='%s' u='%s' limit='%d' selected='%d'>"
			"</bbsmybrd></root>", get_permission(),	currentuser.userid, 
			GOOD_BRC_NUM, num);
	return 0;
}
...
int bbsmybrd_main(void)
{
	...
	xml_header("bbsmybrd");
	printf("<root><bbsmybrd p='%s' u='%s' limit='%d'>", get_permission(),
