...
int main() 
{
	...
	start=atoi(get_param("start"));
	if(strlen(get_param("start"))==0 || start>total-20) 
		start=total-20;
        my_t_lines=atoi(get_param("my_t_lines"));
	if(my_t_lines<10 || my_t_lines>40) my_t_lines=TLINES;
	if(strlen(get_param("start"))==0 || start>total-my_t_lines) start=total-my_t_lines+1;
	if(start<1)start=1;
