...
int b_notes_passwd()
{
  ...
  if (!check_notespasswd())
    return FULLUPDATE;
  getdata(3, 0, "ÇëÊäÈëĞÂµÄÃØÃÜ±¸ÍüÂ¼ÃÜÂë(Enter È¡ÏûÃÜÂë): ", passbuf, 19,
	  NOECHO, YEA);
  if (passbuf[0] == '\0') {
    setvfile(buf, currboard, "notespasswd");
    unlink(buf);
...
/*	count_result(NULL); */
  ...
  if (apply_record(fname, count_result, sizeof(struct ballot), 0, 0, 0) == -1) {
...
int get_vitems(struct votebal *bal)
{
  ...
  for (num = 0; num < 32; num++) {
    sprintf(buf, "%c) ", num + 'A');
    getdata((num % 16) + 4, (num / 16) * 40, buf, bal->items[num], 36, DOECHO,
	    YEA);
...
/*
  ...
  sprintf(buf, " %s%3d %-12.12s %6.6s %-39.39s %-4.4s %3d  %4d[m\n",
	  (voted_flag == NA) ? "[1m" : "", i, ent->userid, datestring + 6,
	  title, vote_type[ent->type - 1], ent->maxdays, num_voted);
//Ended IAMFAT
  prints("%s", buf);
}
...
int vote_key(int ch, int allnum, int pagenum)
{
  ...
  case 'm':
    ...
    substitute_record(controlfile, &currvote, sizeof(struct votebal),
		      allnum + 1);
...
int Show_Votes()
{
  ...
  printvote(NULL);
  setcontrolfile();
  if (apply_record(controlfile, printvote, sizeof(struct votebal), 0, 0, 0) ==
      -1) {
