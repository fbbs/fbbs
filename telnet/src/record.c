...
int get_records(const char *filename, char *rptr, int size,int id,int number)
{
	int     fd;
	int     n;
	if ((fd = open(filename, O_RDONLY, 0)) == -1)
...
int apply_record(	char *filename, 
					APPLY_FUNC_ARG fptr, 
					int size, 
					void *arg, 
					int applycopy, 
...
int	delete_record	(	char *filename,
						int size, 
						int id, 
						RECORD_FUNC_ARG filecheck, 
						void *arg
					)
{
	int		fdr;
...
//插入记录
int insert_record	(	char *filename, 
						int size, 
						RECORD_FUNC_ARG filecheck, 
						void *arg
					)
