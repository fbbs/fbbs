...
char *get_short_date(time_t t) {
    ...
    tm=localtime(&t);
    sprintf(short_date, "%04d.%02d.%02d",
...
void a_showmenu(MENU   *pm) {
    ...
    */
	...
	sprintf(genbuf, "[1;44;31m[°æ  Ö÷]  [33mËµÃ÷ h©¦Àë¿ª q,¡û©¦ÐÂÔöÎÄÕÂ a©¦ÐÂÔöÄ¿Â¼ g©¦±à¼­µµ°¸ E   [%sË¿Â·Ä£Ê½] [m",DEFINE(DEF_MULTANNPATH)?"¶à":"µ¥");
    prints("%s", (pm->level & PERM_BOARDS) ?genbuf :
...
//		memset(litem, 0, sizeof(ITEM));
        ...
        if (strncmp(buf, "Name=", 5) == 0) {
            ...
            if ((!strstr(litem.title,"(BM: BMS)")||chk_currBM(currBM, 0))
                    ...
                    &&(!strstr(litem.title,"(BM: SYSOPS)")
                       ||HAS_PERM(PERM_SYSOPS))
                    &&(strncmp(litem.title,"<HIDE>",6)
                       ||!strncmp(litem.title+6,currentuser.userid,strlen(currentuser.userid)))) {//if
...
int quote_save(char *path,char *key,struct fileheader *fileinfo,int nomsg) {
    FILE        *inf, *outf;
    char        buf[256];
    //, *ptr;
    //char        op;
    ...
    if( dashf( board ) ) {
        ...
        if( ans[0] == 'Y' || ans[0] == 'y' ||nomsg)
            sprintf( genbuf, "/bin/cat bm/bmtmp.%s >> %s", currentuser.userid, board );
        else
            sprintf( genbuf, "/bin/cp -r bm/bmtmp.%s %s", currentuser.userid, board );
    } else {
        sprintf( genbuf, "/bin/cp -r bm/bmtmp.%s  %s", currentuser.userid, board );
    }
    system( genbuf );
...
int a_Import	(	char *path,
    ...
    */
    ...
    sprintf(genbuf, "%s %sÊÕÂ¼ '%-20.20s..'\n", get_short_date(time(0)), currentuser.userid, fileinfo->title);
    ...
    if (key[0] != '\0' && (fn = fopen(buf, "r")) != NULL) {
        while (fgets(buf, sizeof(buf), fn) != NULL) {
            ...
            if (!strcasecmp(buf, key)) {
                ...
                {
			bp = getbcache(key);
			if ((bp->flag & BOARD_CLUB_FLAG)&& (bp->flag & BOARD_READ_FLAG )&& !chk_currBM (bp->BM, 1)&& !isclubmember(currentuser.userid, bp->filename))
						break;
...
void	a_newitem(MENU *pm,int mode) {
    ...
    sprintf(genbuf, "%s %s½¨Á¢%s '%-20.20s..'\n", get_short_date(time(0)), currentuser.userid, (head=='D')?"Ä¿Â¼":"ÎÄÕÂ",title);
    file_append(ANN_LOG_PATH, genbuf);
...
void	a_copypaste(MENU *pm,int paste) {
    ...
    if (!paste) {
	    ...
	    a_additem(pm, title, filename);
            a_savenames(pm);
            //add by fangu 2003.2.26, add log
            sprintf(genbuf, "%s %sÕ³Ìù '%-20.20s..' µ½ '%-20.20s..'\n", get_short_date(time(0)), currentuser.userid, title, pm->mtitle);
            file_append(ANN_LOG_PATH, genbuf);
			bm_log(currentuser.userid, currboard, BMLOG_DOANN, 1);
...
void a_delete( MENU   *pm) {
    ...
    if(!strncmp(genbuf,fpath,sizeof(fpath)))
        a_prompt(-1,"[1;5;31m¾¯¸æ[m: ¸Ãµµ°¸/Ä¿Â¼ÊÇ±»°æÖ÷×öÁË±ê¼Ç, [1;33mÈç¹ûÉ¾³ý, Ôò²»ÄÜÔÙÕ³Ìù¸ÃÎÄÕÂ!![m",ans,2);
    if (dashf(fpath)) {
        ...
        for (i=num1-1;i<=num2-1;i++ ) {
            ...
            if (dashd(genbuf))
                if (strstr(annpath,genbuf)!=NULL) {
                    presskeyfor("ÄúÉè¶¨µÄË¿Â·Î»ÓÚËùÑ¡Ä³¸öÄ¿Â¼ÖÐ£¬»áµ¼ÖÂÇ¶Ì×Ä¿Â¼£¬±¾´Î²Ù×÷È¡Ïû.",t_lines-1);
                    return DONOTHING;
                }
        }
    }
...
a_a_Import(MENU *pm,int msg,int menuitem) {
    ITEM   *item;
    char    fpath[PATHLEN],annpath[512],fname[512],dfname[512],*ip,genbuf[1024];
    int     ch;
    FILE    *fn;
    MENU    newpm;
    ...
    pm->page=9999;
    //add by fangu 2003.2.26, add log
    sprintf(genbuf, "%s %s´Ó '%-20.20s..' ÊÕÂ¼ '%-20.20s..'\n", get_short_date(time(0)), currentuser.userid, pm->mtitle, item->title);
    file_append(ANN_LOG_PATH, genbuf);
	bm_log(currentuser.userid, currboard, BMLOG_DOANN, 1);
        ...
        if (annpath[ipos]=='/' || annpath[ipos+1]=='\0') {
            ...
            while (fgets(buf, sizeof(buf), fn) != NULL) {
                ...
                if (strncmp(buf, "Name=", 5) == 0) {
                    ...
                    if (((strncmp(buf, "Path=~/", 7) == 0) && (strncmp(buf+7,dirname,ipos4)==0))
                            ||(strncmp(buf+5,dirname,ipos4)==0)) {
    ...
    if (pm->num > 0)
        ...
        case 'T':
           ...
           //add end
             ...
             */
            if (*changed_T) {
                sprintf(genbuf, "%s %s½« '%-20.20s..' ¸ÄÃûÎª '%-20.20s..'\n", get_short_date(time(0)), currentuser.userid, item->title, changed_T);
                file_append(ANN_LOG_PATH, genbuf);
				bm_log(currentuser.userid, currboard, BMLOG_DOANN, 1);
                if (dashf(fpath)) {
...
void	a_menu(char *maintitle,char* path,int lastlevel,int lastbmonly) {
    ...
    if(( strstr(me.mtitle, "(BM: BMS)") && !chk_currBM(currBM, 0))
            || (strstr(me.mtitle, "(BM: SYSOPS)") && !HAS_PERM(PERM_SYSOPS))
            || (strstr(me.mtitle, "(BM: OBOARDS)") && !HAS_PERM(PERM_OBOARDS))
            || (strstr(me.mtitle, "(BM: SECRET)") && !chk_currBM(currBM, 0))) {
    ...
    if (me.now < me.num) {
                ...
                if (get_a_boardname(bname, "ÇëÊäÈëÒª×ªÌùµÄÌÖÂÛÇøÃû³Æ: ")) {
                    ...
                    if (askyn(tmp, NA, NA) == 1) {
                        Postfile(fname, bname, me.item[me.now]->title, 4);
                        move(3, 0);
...
int edit_grp(char bname[STRLEN],char grp[STRLEN],char title[STRLEN],char newtitle[STRLEN]) {
    char    buf[STRLEN], buf2[STRLEN], buf3[30];
    char    gpath[STRLEN * 2];
    char    bpath[STRLEN * 2];
    int     i;
...
void Announce() {
    ...
    a_menu(genbuf, "0Announce",
           HAS_PERM(PERM_ANNOUNCE)?PERM_BOARDS: 0, 0
          );
    clear();
}
...
int a_file_info(MENU *pm)
{
	...
	if( dashf( fname ) ) {
		snprintf(weblink, 1024,
				"http://%s/cgi-bin/bbs/bbsanc?path=%s\n",
				BBSHOST, fname + 9);
		strcpy(type,"ÎÄ¼þ");
	}
	else if ( dashd( fname ) ) 
	{
		snprintf(weblink, 1024,
				"http://%s/cgi-bin/bbs/bbs0an?path=%s\n",
				BBSHOST, fname + 9);
		strcpy(type,"Ä¿Â¼");
	}
	else
...
int set_ann_path(char *title, char *path, int mode)
{
	...
	while(1){
			...
			while (y < t_lines-1 && to < MAXANNPATHS)
			{
				if (show_mode == 0)
					sprintf(genbuf, "%4d  %-72.72s",to+1,
							import_path[to].num == -1?"[32m<ÉÐÎ´Éè¶¨>[m":import_path[to].title);
				else if (show_mode == 2)
					sprintf(genbuf, "%4d  %-72.72s",to+1,
							import_path[to].num == -1?"[32m<ÉÐÎ´Éè¶¨>[m":import_path[to].path);
				else if (show_mode = 1){
					strncpy(buf, import_path[to].path, PATHLEN);
					ann_to_title(buf);
					sprintf(genbuf, "%4d  %-72.72s",to+1,
							import_path[to].num == -1?"[32m<ÉÐÎ´Éè¶¨>[m":buf);
				}
		...
		case KEY_PGUP:
			...
			if(curr_annpath<from) curr_annpath=from;
			redrawflag=1;
			break;
		case Ctrl('F'):
		case KEY_PGDN:
		...
		case 'D':	//É¾³ý
			move(1, 0);
			redrawflag=1;
			if(askyn("É¾³ýË¿Â·Âð£¿",NA,NA)==NA) break;
		...
		case 'C':	//Çå³ý
			move(1,0);
			redrawflag=1;
			if(askyn("Çå³ýËùÓÐË¿Â·Âð?",NA,NA)==NA) break;
			clear_ann_path();
			break;
		case 'S':
			sethomefile(genbuf,currentuser.userid,".announcepath");
		...
		case 't'://ÖØÃûÃû
		case 'T'://ÖØÃûÃû
			if(import_path[curr_annpath].num == -1) break;
			if (ann_mode == ANNPATH_SETMODE){
				move(1,0);
				change_ann_path(curr_annpath, import_path[curr_annpath].title, NULL, 1);
			}
			redrawflag=1;
			break;
		case 's':
		...
		case 'G'://½øÈëË¿Â·
			...
			if (ann_mode == ANNPATH_SETMODE){
				if (dashd(import_path[curr_annpath].path)){
					a_menu("", import_path[curr_annpath].path,  PERM_BOARDS, 0);
					return 0;
				}
				else
		...
		case 'M':
			if (import_path[curr_annpath].num != -1){
				...
				if ((buf[0] != 0) && isdigit(buf[0])) {
					...
					new_pos = atoi(buf);
					new_pos--;
					if ((new_pos >= 0) && (new_pos < MAXANNPATHS) && (new_pos != curr_annpath)) {
						if (import_path[new_pos].num != -1){
							strncpy(buf,import_path[curr_annpath].title, STRLEN);
							strncpy(import_path[curr_annpath].title, import_path[new_pos].title,STRLEN);
							strncpy(import_path[new_pos].title, buf, STRLEN);
							
							strncpy(buf,import_path[curr_annpath].path, STRLEN);
							strncpy(import_path[curr_annpath].path, import_path[new_pos].path, STRLEN);
							strncpy(import_path[new_pos].path, buf,STRLEN);
						}else{
							strncpy(import_path[new_pos].title, import_path[curr_annpath].title, STRLEN);
							strncpy(import_path[new_pos].path,  import_path[curr_annpath].path , STRLEN);
		...
		case '\n':
			...
			if (number > 0){
				...
				if (ann_mode == ANNPATH_SETMODE){
					...
					if(import_path[curr_annpath].num != -1){
						sprintf(genbuf,"·ÅÈëË¿Â·%d [%-0.50s]Âð?",import_path[curr_annpath].num + 1, import_path[curr_annpath].title);
						if(askyn(genbuf,YEA,NA)==YEA){
							sethomefile(genbuf,currentuser.userid,".announcepath");
							if(fn = fopen(genbuf,"w+")) {
								fprintf(fn,"%s",import_path[curr_annpath].path);
								fclose(fn);
							}
							return 1;
						}	
