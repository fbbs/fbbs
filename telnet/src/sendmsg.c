...
//»ñÈ¡msg¼ÇÂ¼ÊıÄ¿,Ô­À´µÄget_num_recordÖ»ÄÜÖ§³ÖÒ»ĞĞmsg
...
/* added by roly 02.06.02 for logoff msg */
		...
		//sprintf(msgbuf,"[0;1;45;36m%-12.12s[36mÏòÄú¸æ±ğ([1;36;45m%8.8s[36m)£º[m[1;36;45m%s[m[%05dm\n",
		//	currentuser.userid,timestr, msgstr, 0);
        sprintf(msgbuf, "[0;1;45;36m%-12.12s[36mÏòÄú¸æ±ğ([1;36;45m%24.24s[36m)£º[m[1;36;45m%-38.38s[m[%05dm\n", currentuser.userid, timestr, " ", 0); 
		sprintf(wholebuf, "%s\n", msgstr); 
        strcat(msgbuf, wholebuf);
	}
/* add end */
...
r_msg2()
{
	...
	while (1) {
        ...
        //prints("%s", msg); //¸Ä³É¶àĞĞmsgÒÔºó£¬Ã»ÄÇÃ´¼òµ¥
...
//ÔÚÔÄ¶ÁĞÅÏ¢ÆÚ¼ä£¬ÓÖÓĞÆäËûĞÅÏ¢·¢Èë£¬ÄÇÃ´ÒÔÏûÏ¢µÄĞÎÊ½Í¨Öªmsg_num++
void
count_msg()
{
	signal(SIGUSR2,count_msg);  
...
r_msg()
{
	...
	while (msg_num) {
        ...
        move(line+1, 0);
        clrtoeol();
        msg_line = show_data(msg, LINE_LEN-1, line + 1, 0); //·µ»ØµÄÊÇmsg_lineĞĞ,Ò²¾ÍÊÇ»ØĞÅÏ¢µÄµÚÒ»ĞĞ
