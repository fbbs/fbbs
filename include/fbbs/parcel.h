...
typedef uint32_t parcel_size_t;

enum {
	PARCEL_SIZE_LENGTH = sizeof(parcel_size_t),
};

typedef struct {
	parcel_size_t size;
	parcel_size_t capacity;
	unsigned char *ptr;
	bool error;
} parcel_t;
...
extern parcel_size_t parcel_size(const parcel_t *parcel);
...
extern void parcel_write_string_with_size(parcel_t *parcel, const char *str, parcel_size_t size);
...
extern void parcel_read_new(const char *ptr, parcel_size_t size, parcel_t *parcel);
...
extern const char *parcel_read_string_and_size(parcel_t *parcel, parcel_size_t *size);
