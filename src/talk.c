...
static char *refuse[] = {
		"±§Ç¸£¬ÎÒÏÖÔÚÏë×¨ÐÄ¿´ Board¡£    ",
		"Çë²»Òª³³ÎÒ£¬ºÃÂð£¿..... :)      ",
		"ÎÒÏÖÔÚÓÐÊÂ£¬µÈÒ»ÏÂÔÙ Call Äã¡£  ",
		"ÎÒÂíÉÏÒªÀë¿ªÁË£¬ÏÂ´ÎÔÙÁÄ°É¡£    ",
		"ÇëÄã²»ÒªÔÙ Page£¬ÎÒ²»Ïë¸úÄãÁÄ¡£ ",
		"ÇëÏÈÐ´Ò»·â×ÔÎÒ½éÉÜ¸øÎÒ£¬ºÃÂð£¿  ",
		"¶Ô²»Æð£¬ÎÒÏÖÔÚÔÚµÈÈË¡£          ",
		"ÎÒ½ñÌìºÜÀÛ£¬²»Ïë¸ú±ðÈËÁÄÌì¡£    ",
		NULL
};
...
int tui_query_result(const char *userid)
{
	...
	if (self || HAS_PERM2(PERM_OCHAT, &currentuser)) {
		...
		prints("¹â»ª±Ò [\033[1;32m%d\033[m] ", TO_YUAN_INT(money));
	}
#endif
#ifdef ALLOWGAME
	prints("´æ¿î [\033[1;32m%dÔª\033[m] ´û¿î [\033[1;32m%dÔª\033[m]"
