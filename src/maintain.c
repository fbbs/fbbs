...
int kick_user(void)
{
	...
	move(1, 0);
	clrtoeol();
	snprintf(buf, sizeof(buf), "ÃﬂµÙ π”√’ﬂ : [%s].", user);
	move(2, 0);
	if (!askyn(buf, NA, NA)) {
		presskeyfor("»°œ˚Ãﬂ π”√’ﬂ..", 2);
		clear();
		return 0;
	}
	...
	if (!search_ulist(&uin, t_cmpuids, uid) || do_kick_user(&uin) < 0) {
		move(3, 0);
		presskeyfor("∏√”√ªß≤ª‘⁄œﬂ…œªÚŒﬁ∑®Ãﬂ≥ˆ’æÕ‚..", 3);
		clear();
		return 0;
	}
	presskeyfor("∏√”√ªß“—æ≠±ªÃﬂœ¬’æ£®»Á∏√”√ªß”–∂‡Ω¯≥Ã«Î÷ÿ∏¥±æ≤Ÿ◊˜£©", 4);
	clear();
	return 1;
}
