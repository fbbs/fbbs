...
int read_post(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (digestmode == ATTACH_MODE) {
		...
		if (haspostperm(&currentuser, currbp)) {
			prints("\033[0;1;44;31m[ÔÄ¶ÁÎÄÕÂ]  \033[33m»ØÐÅ R ©¦ ½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,¡ý©¦Ö÷ÌâÔÄ¶Á ^s»òp \033[m");
		} else {
			prints("\033[1;44;31m[ÔÄ¶ÁÎÄÕÂ]  \033[33m½áÊø Q,¡û ©¦ÉÏÒ»·â ¡ü©¦ÏÂÒ»·â <Space>,<Enter>,¡ý©¦Ö÷ÌâÔÄ¶Á p       \033[m");
		}
		...
		if (mmap_open(orig, &m) == 0) {
			...
			while (1) {
				...
				if (!header || mode == 'A') {
					...
					if (mode == 'S' && lend - ptr > 10 + sizeof("¡ù À´Ô´:¡¤")
							&& !memcmp(ptr + 10, "¡ù À´Ô´:¡¤", sizeof("¡ù À´Ô´:¡¤"))) {
						break;
					}
					if (mode == 'R') {
						bytes += lend - ptr;
...
// Çø¶ÎÉ¾³ý,´Óid1µ½id2;°æÃæÎÄ¼þÓë¾«»ªÇøÓÐÇø±ð
...
int read_trash(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (!dashf(currdirect)) {
		digestmode = NA;
		setbdir(currdirect, currboard);
		presskeyfor("°æÃæÀ¬»øÏäÎÞÄÚÈÝ£¬°´ÈÎÒâ¼ü¼ÌÐø...", t_lines-1);
		update_endline();
		return DONOTHING;
	}
	return NEWDIRECT;
...
int read_attach(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (!dashf(currdirect)) {
		digestmode = NA;
		setbdir(currdirect, currboard);
		presskeyfor("°æÃæ¸½¼þÇøÎÞÄÚÈÝ£¬°´ÈÎÒâ¼ü¼ÌÐø...", t_lines-1);
		update_endline();
		return DONOTHING;
	}
	return NEWDIRECT;
...
int read_junk(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (!dashf(currdirect)) {
		digestmode = NA;
		setbdir(currdirect, currboard);
		presskeyfor("Õ¾ÎñÀ¬»øÏäÎÞÄÚÈÝ£¬°´ÈÎÒâ¼ü¼ÌÐø...", t_lines-1);
		update_endline();
		return DONOTHING;
	}
	return NEWDIRECT;
