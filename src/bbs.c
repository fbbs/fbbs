...
char *readdoent(int num, struct fileheader *ent) //Post list
{
	...
	if (digestmode == TRASH_MODE || digestmode == JUNK_MODE) {
		...
		if (ent->accessed[1] & FILE_NOTICE) {
			snprintf(buf, sizeof(buf), " \033[1;31m [¡Ş]\033[m %c \033[%sm"
...
int read_trash(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (!dashf(currdirect)) {
		digestmode = NA;
		setbdir(currdirect, currboard);
		presskeyfor("°æÃæÀ¬»øÏäÎŞÄÚÈİ£¬°´ÈÎÒâ¼ü¼ÌĞø...", t_lines-1);
		update_endline();
		return DONOTHING;
	}
	return NEWDIRECT;
...
int read_junk(int ent, struct fileheader *fileinfo, char *direct) {
	...
	if (!dashf(currdirect)) {
		digestmode = NA;
		setbdir(currdirect, currboard);
		presskeyfor("Õ¾ÎñÀ¬»øÏäÎŞÄÚÈİ£¬°´ÈÎÒâ¼ü¼ÌĞø...", t_lines-1);
		update_endline();
		return DONOTHING;
	}
	return NEWDIRECT;
