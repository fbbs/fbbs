...
int giveUpBBS() {
	...
	getdata(10, 0, "ÇëÑ¡Ôñ [0]", ans, 2, DOECHO, NA);
	if (ans[0] < '1' || ans[0] > '4') {
		return 0;
	}
	k = 1;
	...
	getdata(11, 0, "ÇëÊäÈë½äÍøÌìÊı [0]", day, 4, DOECHO, NA);
	i = 0;
	while (day[i]) {
		if (!isdigit(day[i]))
			return 0;
	...
	if (askyn("ÄãÈ·¶¨Òª½äÍøÂğ£¿", NA, NA) == 1) {
		getdata(15, 0, "ÇëÊäÈëÃÜÂë: ", buf, 39, NOECHO, NA);
